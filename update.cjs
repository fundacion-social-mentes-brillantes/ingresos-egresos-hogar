...
// IMPORTS
const importsToAdd = `import { useAuth } from '../contexts/AuthContext';
...
content = content.replace(
  ...
  'const [selectedAccountId, setSelectedAccountId] = useState<string | null>(null);' + stateCode
);

// TRANSFER BUTTON
const btnCode = `<div className="flex items-center justify-between">
...
content = content.replace('{selectedAccount ? (', formCode);
