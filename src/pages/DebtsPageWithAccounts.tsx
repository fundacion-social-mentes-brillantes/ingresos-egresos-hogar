...
import { AlertCircle, CalendarClock, CheckCircle2, HandCoins, Loader2, Plus, Trash2 } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { deleteDebt, getAccounts } from '../lib/firestore';
...
export function DebtsPageWithAccounts() {
  ...
  const loadAccounts = async () => {
    ...
    setAccounts(items.filter((account) => account.active));
  };
    ...
    <div className="mx-auto w-full max-w-6xl space-y-6 pb-10">
      <section className="lux-hero relative p-5 sm:p-7">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="grid gap-3 sm:grid-cols-3 lg:min-w-[520px]">
            ...
            <div className="rounded-3xl border border-red-500/20 bg-red-500/10 p-4"><p className="text-xs font-black uppercase tracking-[0.16em] text-red-300">Tu debes</p><p className="mt-1 text-xl font-black text-slate-100">{formatCOP(summary.payable)}</p></div>
            <div className="rounded-3xl border border-blue-500/20 bg-blue-500/10 p-4"><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-300">Balance neto</p><p className="mt-1 text-xl font-black text-slate-100">{formatCOP(summary.net)}</p></div>
          </div>
        </div>
      </section>
      ...
      <div className="grid gap-5 xl:grid-cols-[390px_1fr]">
        <section className="lux-card p-5">
          <div className="mb-4 flex items-center gap-2"><div className="premium-icon h-10 w-10 text-blue-200"><Plus className="h-5 w-5" /></div><div><h2 className="font-black text-slate-100">Añadir deuda</h2><p className="text-xs text-slate-500">Registro manual con movimiento de cuenta</p></div></div>
          <div className="space-y-3">
            ...
            <input className="lux-input w-full rounded-2xl px-4 py-3 text-sm outline-none" placeholder="Descripcion: prestamo, almuerzo, arriendo..." value={form.description} onChange={(event) => setForm((prev) => ({ ...prev, description: event.target.value }))} />
            ...
            <button onClick={handleCreate} disabled={saving} className="premium-button flex w-full items-center justify-center gap-2 rounded-2xl px-4 py-3 text-sm font-black transition disabled:opacity-50">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />}Guardar deuda y mover cuenta</button>
          </div>
        </section>
        ...
        <section className="lux-card p-5">
          ...
          {loading ? <div className="flex h-48 items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-blue-400" /></div> : filteredDebts.length === 0 ? <EmptyState asset="debts" title="No hay deudas en esta vista" description="Puedes crearlas manualmente o decirle al copiloto: Juan me debe 50 mil desde Nequi." /> : <div className="grid gap-3">{filteredDebts.map((debt) => {
